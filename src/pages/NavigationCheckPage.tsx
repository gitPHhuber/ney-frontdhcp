...
const NavigationCheckPage = () => {
    const [diagnostics, setDiagnostics] = useState<DiagnosticResult[]>([]);
    const [isRunning, setIsRunning] = useState(false);
    const [lastRun, setLastRun] = useState<string | null>(null);

    const summary = useMemo(() => diagnostics.reduce((acc, item) => {
        acc[item.status] += 1;
        return acc;
    }, {
        success: 0,
        warning: 0,
        error: 0,
    } as Record<DiagnosticStatus, number>), [diagnostics]);
        ...
        <div className="navigation-check-page">
            <header className="page-header">
                <div className="page-header-copy">
                    ...
                    {lastRun && (
                        <p className="last-run">Last run: {formatDateTime(lastRun)}</p>
                    )}
                </div>
